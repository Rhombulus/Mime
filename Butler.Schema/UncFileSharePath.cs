        ...
        protected override bool ParseCore(string path, bool nothrow) {
            if (base.ParseCore(path, nothrow)) {
                if (!this.IsUnc) {
                    this.IsValid = false;
                    if (!nothrow)
                        throw new ArgumentException(Resources.DataStrings.ErrorUncPathMustBeUncPath(path), nameof(path));
                } else {
                    if (path.Length >= 260)
                        throw new System.IO.PathTooLongException(Resources.DataStrings.ErrorUncPathTooLong(path));
                    var match = _regexUncShare.Match(this.PathName);
                    if (!match.Success) {
                        this.IsValid = false;
                        if (!nothrow)
                            throw new ArgumentException(Resources.DataStrings.ErrorUncPathMustBeUncPathOnly(path), nameof(path));
                    } else {
                        System.Net.IPAddress address;
                        if (System.Net.IPAddress.TryParse(match.Groups[1].ToString(), out address)) {
                            this.IsValid = false;
                            if (!nothrow)
                                throw new ArgumentException(Resources.DataStrings.ErrorUncPathMustUseServerName(path), nameof(path));
                        }
                    ...
                    if (_regexTrailingWhitespace.Match(this.PathName)
                        ...
                        this.IsValid = false;
                        if (!nothrow)
                            throw new ArgumentException(Resources.DataStrings.ConstraintViolationNoLeadingOrTrailingWhitespace, nameof(path));
                    }
                }
            }
            return this.IsValid;
