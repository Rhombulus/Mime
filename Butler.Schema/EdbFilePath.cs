    ...
    protected override bool ParseCore(string path, bool nothrow)
    {
      if (base.ParseCore(path, nothrow))
      {
        if (this.IsTemporaryEdbFile)
        {
          this.IsValid = false;
          if (!nothrow)
            throw new ArgumentException(Resources.DataStrings.ErrorEdbFileCannotBeTmp(path), nameof(path));
        }
        else
        {
          ...
          catch (FormatException ex)
          {
            this.IsValid = false;
            if (!nothrow)
              throw new ArgumentException(Resources.DataStrings.ErrorEdbFileNameTooLong(fileName), (Exception) ex);
          }
        }
      }
      if (!this.IsValid && !nothrow)
        throw new ArgumentException(Resources.DataStrings.ErrorEdbFilePathCannotConvert(path));
      return this.IsValid;
    }
