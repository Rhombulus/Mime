                            ...
                            case 61:
                                ...
                                if (int.MaxValue != countLimit || int.MaxValue != bytesLimit) {
                                    if (++actual > countLimit)
                                        throw new MimeException(Resources.Strings.TooManyParameters(actual, countLimit));
                                    if (list.Length > bytesLimit)
                                        throw new MimeException(Resources.Strings.TooManyTextValueBytes(list.Length, bytesLimit));
                                    break;
                                }
                                var flag2 = false;
                                var num2 = -1;
