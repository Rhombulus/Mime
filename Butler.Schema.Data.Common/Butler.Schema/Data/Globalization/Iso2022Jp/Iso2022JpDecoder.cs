...
namespace Butler.Schema.Data.Globalization.Iso2022Jp {
    ...
    internal class Iso2022JpDecoder : System.Text.Decoder {
        ...
        public override void Convert(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex, int charCount, bool flush, out int bytesUsed, out int charsUsed, out bool completed) {
            switch (killSwitch) {
                ...
                case Iso2022DecodingMode.Override:
                    ...
                    Array.Copy(bytes, sourceIndex, leftovers, 0, length);
                    break;
                case Iso2022DecodingMode.Throw:
                    throw new NotImplementedException();
                default:
            ...
            while (byteCount > 0) {
                ...
                if (bytesConsumed == 0 && bytes[byteIndex] == 0) {
                    ...
                    if (bytesConsumed != usedIn)
                        throw new InvalidOperationException(string.Format("{2}.GetNextRun input length {0} does not match ConvertToCp932 length {1}", bytesConsumed, usedIn, subDecoders[currentSubDecoder].GetType()));
                    if (usedOut != cp932ByteCount)
                        throw new InvalidOperationException(string.Format("{2}.GetNextRun output length {0} does not match ConvertToCp932 length {1}", cp932ByteCount, usedOut, subDecoders[currentSubDecoder].GetType()));
        ...
        public override string ToString() {
            return string.Format("{0} runs; {1}", runCount, sbSummary.ToString());
        }
