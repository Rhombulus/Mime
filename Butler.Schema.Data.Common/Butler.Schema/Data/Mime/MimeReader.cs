    ...
    public class MimeReader : IDisposable {
        ...
        private void ParseAndCheckSize() {
            ...
            if (parser.Position > this.MimeLimits.MaxSize)
                throw new MimeException(CtsResources.Strings.InputStreamTooLong(parser.Position, this.MimeLimits.MaxSize));
        }
        ...
        private void CheckHeaderBytesLimits() {
            ...
            if (cumulativeHeaderBytes > this.MimeLimits.MaxHeaderBytes)
                throw new MimeException(CtsResources.Strings.TooManyHeaderBytes(cumulativeHeaderBytes, this.MimeLimits.MaxHeaderBytes));
            ...
            if ((type == typeof (TextHeader) || type == typeof (AsciiTextHeader)) && currentTextHeaderBytes > this.MimeLimits.MaxTextValueBytesPerValue)
                throw new MimeException(CtsResources.Strings.TooManyTextValueBytes(currentTextHeaderBytes, this.MimeLimits.MaxTextValueBytesPerValue));
        }

        private void CheckPartsLimit() {
            if (++partCount > this.MimeLimits.MaxParts)
                throw new MimeException(CtsResources.Strings.TooManyParts(partCount, this.MimeLimits.MaxParts));
            ...
            if (embeddedDepth > this.MimeLimits.MaxEmbeddedDepth)
                throw new MimeException(CtsResources.Strings.EmbeddedNestingTooDeep(embeddedDepth, this.MimeLimits.MaxEmbeddedDepth));
        }
        ...
        private void StartHeader() {
            if (++headerCount > this.MimeLimits.MaxHeaders)
                throw new MimeException(CtsResources.Strings.TooManyHeaders(headerCount, this.MimeLimits.MaxHeaders));
