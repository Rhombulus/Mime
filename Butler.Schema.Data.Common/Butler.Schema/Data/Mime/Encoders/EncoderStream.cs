    ...
    public class EncoderStream : System.IO.Stream, Internal.ICloneableStream {
        ...
        public EncoderStream(System.IO.Stream stream, ByteEncoder encoder, EncoderStreamAccess access) {
            ...
            if (access == EncoderStreamAccess.Read) {
                if (!stream.CanRead)
                    throw new NotSupportedException(CtsResources.EncodersStrings.EncStrCannotRead);
            } else if (!stream.CanWrite)
                throw new NotSupportedException(CtsResources.EncodersStrings.EncStrCannotWrite);
            this.stream = stream;
        ...
        public System.IO.Stream Clone() {
            this.AssertOpen();
            if (EncoderStreamAccess.Write == access)
                throw new NotSupportedException(CtsResources.EncodersStrings.EncStrCannotCloneWriteableStream);
            ...
            if (cloneableStream == null) {
                throw new NotSupportedException(
                    CtsResources.EncodersStrings.EncStrCannotCloneChildStream(
                        stream.GetType()
                              .ToString()));
            }
        ...
        public override sealed int Read(byte[] array, int offset, int count) {
            ...
            if (offset + count > array.Length)
                throw new ArgumentOutOfRangeException("offset, count", CtsResources.EncodersStrings.EncStrLengthExceeded(offset + count, array.Length));
            if (0 > offset || 0 > count)
        ...
        public override sealed void Write(byte[] array, int offset, int count) {
            ...
            if (count + offset > array.Length)
                throw new ArgumentException(CtsResources.EncodersStrings.EncStrLengthExceeded(offset + count, array.Length), nameof(array));
            if (0 > offset || 0 > count)
                throw new ArgumentOutOfRangeException(offset < 0 ? "offset" : "count");
            while (count != 0) {
                int inputUsed;
        ...
        public override sealed long Seek(long offset, System.IO.SeekOrigin origin) {
            this.AssertOpen();
            if (!this.CanSeek)
                throw new NotSupportedException(CtsResources.EncodersStrings.EncStrCannotSeek);
