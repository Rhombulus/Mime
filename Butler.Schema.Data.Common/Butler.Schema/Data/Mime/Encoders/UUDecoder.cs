...
namespace Butler.Schema.Data.Mime.Encoders {
    ...
    public class UUDecoder : ByteEncoder {
        ...
        public string FileName {
            get {
                if (fileName != null)
                    return CTSGlobals.AsciiEncoding.GetString(fileName, 0, fileName.Length);
                return string.Empty;
            }
        }
        ...
        public override sealed void Convert(byte[] input, int inputIndex, int inputSize, byte[] output, int outputIndex, int outputSize, bool flush, out int inputUsed, out int outputUsed, out bool completed) {
            ...
            if (decodedSize != 0) {
                var num2 = Math.Min(outputSize, decodedSize);
                if ((num2 & 2) != 0) {
            ...
            do {
                if (lineReady) {
                    var num2 = Math.Min(outputSize/3, encodedBytes/3);
                    ...
                    if (0 < outputSize && 0 < encodedBytes) {
                        ...
                        numArray[2] = UUDecoder.UUDecode(encoded[encodedIndex + 2]);
                        numArray[3] = UUDecoder.UUDecode(encoded[encodedIndex + 3]);
                        decodedSize = Math.Min(encodedBytes, 3);
                        decodedIndex = 0;
                        ...
                        encodedIndex += 4;
                        var num3 = Math.Min(outputSize, decodedSize);
                        if ((num3 & 2) != 0) {
                            output[outputIndex++] = decoded[decodedIndex++];
                            output[outputIndex++] = decoded[decodedIndex++];
                        }
                ...
                while (num1 != inputIndex || flush && encodedSize != 0) {
                    ...
                    if (10 != num2) {
                        if (encodedSize == encoded.Length)
                            throw new ByteEncoderException(CtsResources.EncodersStrings.UUDecoderInvalidData);
                        encoded[encodedSize++] = num2;
                    } else {
                        if (!this.UULineGood())
                            throw new ByteEncoderException(CtsResources.EncodersStrings.UUDecoderInvalidDataBadLine);
                        if (encodedBytes != 0) {
                            lineReady = true;
                            break;
                        }
