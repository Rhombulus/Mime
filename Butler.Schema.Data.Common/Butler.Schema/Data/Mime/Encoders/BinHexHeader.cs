...
namespace Butler.Schema.Data.Mime.Encoders {
    ...
    internal class BinHexHeader {
        ...
        public BinHexHeader(byte[] header) {
            if (header.Length < 23)
                throw new ArgumentException(CtsResources.EncodersStrings.BinHexHeaderTooSmall, nameof(header));
            int length = header[0];
            if (header.Length - 22 < length)
                throw new ByteEncoderException(CtsResources.EncodersStrings.BinHexHeaderIncomplete);
            if (63 < length || 1 > length)
                throw new ByteEncoderException(CtsResources.EncodersStrings.BinHexHeaderInvalidNameLength);
            var index = 2 + length;
            var num1 = BinHexUtils.CalculateHeaderCrc(header, index + 18);
            var num2 = BinHexUtils.UnmarshalUInt16(header, index + 18);
            if (num1 != num2)
                throw new ByteEncoderException(CtsResources.EncodersStrings.BinHexHeaderInvalidCrc);
            if (header[1 + length] != 0)
                throw new ByteEncoderException(CtsResources.EncodersStrings.BinHexHeaderUnsupportedVersion);
        ...
        public BinHexHeader(MacBinaryHeader header) {
            if (63 < header.FileNameLength || 1 > header.FileNameLength)
                throw new ByteEncoderException(CtsResources.EncodersStrings.BinHexHeaderBadFileNameLength);
        ...
        public string FileName {
            get {
                return CTSGlobals.AsciiEncoding.GetString(fileName, 0, this.FileNameLength);
            }
            set {
