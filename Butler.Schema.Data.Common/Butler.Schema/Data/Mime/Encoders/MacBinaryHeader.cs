...
namespace Butler.Schema.Data.Mime.Encoders {
    ...
    public class MacBinaryHeader {
        ...
        public MacBinaryHeader(byte[] bytes) {
            ...
            if (bytes.Length != 128)
                throw new ArgumentException(CtsResources.EncodersStrings.MacBinHeaderMustBe128Long, nameof(bytes));
            if (bytes[0] != 0 || bytes[74] != 0 || bytes[82] != 0)
                throw new ByteEncoderException(CtsResources.EncodersStrings.MacBinInvalidData);
            if (bytes[1] > 63)
                throw new ByteEncoderException(CtsResources.EncodersStrings.MacBinInvalidData);
            this.FileNameLength = bytes[1];
            fileName = CTSGlobals.AsciiEncoding.GetString(bytes, 2, this.FileNameLength);
        ...
        public string FileName {
            ...
            set {
                var bytes = CTSGlobals.AsciiEncoding.GetBytes(value);
                if (bytes.Length > 63)
                    throw new ArgumentException(CtsResources.EncodersStrings.MacBinFileNameTooLong, nameof(value));
                fileName = value;
        ...
        public int XIcon {
            ...
            set {
                if (ushort.MaxValue < (uint) value)
                    throw new ArgumentOutOfRangeException(nameof(value), CtsResources.EncodersStrings.MacBinIconOffsetTooLarge(ushort.MaxValue));
                iconXOffset = value;
            }
        }
        ...
        public int Version {
            ...
            set {
                if (value != 0 && 129 != value && 130 != value)
                    throw new ArgumentOutOfRangeException(nameof(value), CtsResources.EncodersStrings.MacBinBadVersion);
                version = value;
            }
        }
        ...
        private byte[] FileNameAsByteArray() {
            return CTSGlobals.AsciiEncoding.GetBytes(fileName);
        }
